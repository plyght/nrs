...
const MarkdownRenderer = ({ content }: { content: string }) => {
  ...
  useEffect(() => {
    ...
    const parseMarkdown = (md: string) => {
      ...
      processedMd = processedMd.replace(/Back to Index/g, '');

      // Process YAML-like syntax in the content body (e.g., "---" section markers)
      processedMd = processedMd.replace(/^---\s*$/gm, '');

      // Remove duplicate note titles - more aggressive handling
      if (frontMatterMatch && frontMatterMatch[1]) {
        const titleMatch = frontMatterMatch[1].match(/title:\s*(.*?)$/m);
        if (titleMatch && titleMatch[1]) {
          const titleText = titleMatch[1].trim();
          // Remove any markdown headings with the same title (with or without # prefix)
          processedMd = processedMd.replace(new RegExp(`^# ${titleText}\\s*$`, 'gm'), '');
          processedMd = processedMd.replace(new RegExp(`^${titleText}\\s*$`, 'gm'), '');
        }
      }
      
      // Always remove the first h1 heading since we display it separately
      ...
      // Bold and italic
      processedMd = processedMd.replace(/\*\*(.*?)\*\*/gim, '<strong>$1</strong>');
      ...
      // Code blocks
      processedMd = processedMd.replace(/```([^`]*?)```/gim, '<pre><code>$1</code></pre>');

      // Inline code
      processedMd = processedMd.replace(/`([^`]*?)`/gim, '<code>$1</code>');

      // Blockquotes
      processedMd = processedMd.replace(/^\> (.*$)/gim, '<blockquote>$1</blockquote>');
      ...
      processedMd = processedMd.replace(
        /^\* \[ \] (.*)/gm,
        '<div class="flex items-center gap-2 py-1"><input type="checkbox" class="form-checkbox h-5 w-5 text-primary-500" disabled /><span class="text-gray-700 dark:text-gray-300">$1</span></div>'
      );
      processedMd = processedMd.replace(
        /^\* \[x\] (.*)/gm,
        '<div class="flex items-center gap-2 py-1"><input type="checkbox" class="form-checkbox h-5 w-5 text-primary-500" disabled checked /><span class="text-gray-700 dark:text-gray-300 line-through">$1</span></div>'
      );

      // Improved handling for regular list items (like "* dog food")
      processedMd = processedMd.replace(
        /^\* ([^[].*)/gm,
        '<div class="flex items-center gap-2 py-1 ml-2"><span class="h-2 w-2 rounded-full bg-primary-500 dark:bg-primary-400 inline-block mr-2"></span><span class="text-gray-700 dark:text-gray-300">$1</span></div>'
      );
  ...
  const isEmptyContent =
    html.trim() === '' || (html.includes('Write your note here') && html.length < 50);
  ...
  return (
    <div className="mt-0">
      ...
      {frontMatter && Object.keys(frontMatter).length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-gray-500 dark:text-gray-400 mb-4">
      ...
      {cleanedHtml.trim() !== '' && (
        <div
          ...
          dangerouslySetInnerHTML={{ __html: cleanedHtml.replace(/<h1>.*?<\/h1>/, '')}}
        />
      )}
    </div>
  );
